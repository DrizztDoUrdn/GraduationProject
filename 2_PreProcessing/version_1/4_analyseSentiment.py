...
def analyze_sentiment_and_save(input_file_path, lexicon, output_file_path):
    ...
    df = pd.read_csv(input_file_path, sep=';', header=None, names=['text'])
...
for filename in os.listdir(csv_file_dir):
    if filename.endswith(".csv"):
        ...
        output_file_path = os.path.join(output_dir, f"analysed_urun_{file_number}.csv")  # Change the filename format
