...
output_dir = '/Data/amazon/4_NormalizedSentiment'
...
for filename in os.listdir(csv_file_dir):
    if filename.endswith("stn_main.csv"): ##Burayi duzelt
        input_file_path = os.path.join(csv_file_dir, filename)
        output_file_path = os.path.join(output_dir, f"normalized_amazon_stn_main.tsv")
